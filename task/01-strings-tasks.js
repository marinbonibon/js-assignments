...
function encodeToRot13 (str) {
  ...
  let startCode = 0,
       endCode = 0;
  const result = split.map((char, i) => {
    const currentCharCode = char.charCodeAt(0);
    if (currentCharCode >= upperCaseStart && currentCharCode <= upperCaseEnd) {
      startCode = upperCaseStart;
      endCode = upperCaseEnd;
    } else if (currentCharCode >= lowerCaseStart && currentCharCode <= lowerCaseEnd) {
      startCode = lowerCaseStart;
      endCode = lowerCaseEnd;
    } else {
      return split[i];
    }
    const resultCharCode = currentCharCode + 13;
    if (resultCharCode > endCode) {
      return String.fromCharCode(resultCharCode - 26);
    } else {
